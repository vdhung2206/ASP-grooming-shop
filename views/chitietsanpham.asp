    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if(obj.data.sltonkho>0){
              str += "<a onclick=\"themgiohang('" + obj.data.masp + "','" + obj.data.giasp +"','" + obj.data.hinhanh + "')\" class=\"btnthemsp btn btn-primary btn-center\">Thêm vào giỏ hàng</a>"
              str +="<input value=\"1\" min=1 max=10 id=\"soluongsp\"type = \"number\" class=\"form-control\" style=\"display: inline-block; width:10%\; padding-top:2px; margin-left:10px\">"
            }
            else{
              str += "Đã hết hàng"
            }
